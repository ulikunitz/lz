...
// ODHSConfig provides the confifuration parameters for the DoubleHashSequencer.
type ODHSConfig struct {
	// maximal window size
	WindowSize int
...
func (cfg *ODHSConfig) Verify() error {
...
// to zero.
func (cfg *ODHSConfig) ApplyDefaults() {
	if cfg.BlockSize == 0 {
...
func (cfg ODHSConfig) NewInputSequencer() (s InputSequencer, err error) {
	return NewOldDoubleHashSequencer(cfg)
}

// OldDoubleHashSequencer generates LZ77 sequences by using two hash tables. The
...
type OldDoubleHashSequencer struct {
	seqBuffer
...
func (s *OldDoubleHashSequencer) MemSize() uintptr {
...
// NewOldDoubleHashSequencer allocates a new DoubleHashSequencer value and
// initializes it. The function returns the first error found in the
// configuration.
func NewOldDoubleHashSequencer(cfg ODHSConfig) (s *OldDoubleHashSequencer, err error) {
	s = new(OldDoubleHashSequencer)
...
func (s *OldDoubleHashSequencer) Init(cfg ODHSConfig) error {
	cfg.ApplyDefaults()
	var err error
...
func (s *OldDoubleHashSequencer) Reset() {
	s.seqBuffer.Reset()
	s.h1.reset()
	s.h2.reset()
	s.pos = 0
...
func (s *OldDoubleHashSequencer) RequestBuffer() int {
...
func (s *OldDoubleHashSequencer) hashSegment1(a, b int) {
	if a < 0 {
		a = 0
	}
	n := len(s.data)
...
func (s *OldDoubleHashSequencer) hashSegment2(a, b int) {
	if a < 0 {
		a = 0
	}
	n := len(s.data)
...
func (s *OldDoubleHashSequencer) Sequence(blk *Block, flags int) (n int, err error) {
	n = s.buffered()
	if blk == nil {
		if n == 0 {
