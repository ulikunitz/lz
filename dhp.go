...
// NewDoubleHashParser creates a new DoubleHashParser.
func NewDoubleHashParser(h1cfg, h2cfg HashConfig, bcfg BufConfig) (Parser, error) {
	dhp := new(doubleHashParser)
	if err := dhp.init(h1cfg, h2cfg, bcfg); err != nil {
		return nil, err
	}
	return dhp, nil
}
