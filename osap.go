...
// XZCost models the cost of the bits going into the XZ encoding. The maximum
// edge length is 273.
...
func NewOptimizedSuffixArrayParser(cfg OSAPConfig, bcfg BufConfig) (Parser, error) {
	osas := new(optSuffixArrayParser)
	if err := osas.init(cfg, bcfg); err != nil {
		return nil, err
	}
	return osas, nil
}
...
func (s *optSuffixArrayParser) init(cfg OSAPConfig, bcfg BufConfig) error {
	cfg.setDefaults()
	var err error
	...
	if s.WindowSize != s.BufferSize {
		s.WindowSize = s.BufferSize
	}
