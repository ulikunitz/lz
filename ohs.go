...
// OldHashSequencer allows the creation of sequence blocks using a simple hash
// table.
type OldHashSequencer struct {
	seqBuffer
...
func (s *OldHashSequencer) MemSize() uintptr {
...
// OHSConfig provides the configuration parameters for the
// HashSequencer.
//
...
func (cfg *OHSConfig) ApplyDefaults() {
	if cfg.BlockSize == 0 {
...
func (cfg *OHSConfig) Verify() error {
...
func (cfg OHSConfig) NewInputSequencer() (s InputSequencer, err error) {
	return NewOldHashSequencer(cfg)
}

// NewOldHashSequencer creates a new hash sequencer.
func NewOldHashSequencer(cfg OHSConfig) (s *OldHashSequencer, err error) {
	var t OldHashSequencer
...
func (s *OldHashSequencer) Init(cfg OHSConfig) error {
	cfg.ApplyDefaults()
	var err error
...
// provided.
func (s *OldHashSequencer) RequestBuffer() int {
...
func (s *OldHashSequencer) hashSegment(a, b int) {
	if a < 0 {
		a = 0
	}
	n := len(s.data)
...
func (s *OldHashSequencer) Sequence(blk *Block, flags int) (n int, err error) {
	n = s.buffered()
	if blk == nil {
		if n == 0 {
